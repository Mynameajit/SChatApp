...
import { TextField, Button, Typography, Box, Paper, AppBar, Toolbar, Avatar, IconButton, Drawer } from "@mui/material";
import DeleteIcon from "@mui/icons-material/Delete";
import { Call, VideoCall } from "@mui/icons-material";
import { Link } from "react-router-dom";
import CallSystem from "./CallSystem";
import VideoCallSystem from "./VideoCallSystem.jsx";

const ChatPage = () => {
  const [messages, setMessages] = useState([]);
  const [newMessage, setNewMessage] = useState("");
  const [isCall, setIsCall] = useState(false);
  const [videoCall, setIsVideoCall] = useState(false);
    ...
    <Box sx={{ display: "flex", flexDirection: "column", height: "100vh", backgroundColor: "#f0f2f5" }}>
      ...
      <AppBar position="static" sx={{ backgroundColor: "#007bff", color: "#ffffff" }}>
        <Toolbar>
          ...
          <IconButton color="inherit" onClick={() => setIsCall(true)}>
            <Call />
          </IconButton>


          <IconButton color="inherit" onClick={() => setIsVideoCall(true)}>
            <VideoCall />
          </IconButton>



          <Avatar sx={{ backgroundColor: "#ffffff", color: "#007bff", marginRight: 0, marginLeft:2}}>
